...
open F1, "<2_M.bolletii_20230801" or die $!; #open Tuberculist_new_20150307
while(<F1>){
	chomp;
	@a=split "\t",$_;
	$start{$a[0]}=$a[3];
...
close F1;
$last_igr="I3U44_RS26420-I3U44_RS00005";
$igrha{$last_igr}=1;
push(@igr,$last_igr);
...
open F3, "<MAB.subsp.bolletii.fna" or die $!; #open genome.fasta
while(<F3>){
	chomp;
	if($_=~m/>/){
		}
...
$type="";				
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				@b=split "-",$j;
				if($j eq "I3U44_RS26420-I3U44_RS00005"){
				$left=$a[0]-$start{$j};
				$right=5066832-$a[0];
				}else{
				$left=$a[0]-$end{$b[0]};
